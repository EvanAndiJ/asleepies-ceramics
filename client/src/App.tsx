...
import { SkuInfo, UserLogin } from './types';
...
function App() {
  ...
  //@ts-expect-error
  const onRemoveFromCart = async (newCart) => {
    setCartCount(cartCount - 1)
    setCart(newCart)
  };
  ...
  return (
    <div id="appDiv">
      ...
      <Link to="/">
        <img id='logoHead' 
          src={`https://tiffanyteaze.com/asleepies/logo.png` } />
      </Link>
      <h3>Handmade Ceramics</h3>

      <NavBar isLoggedIn={userInfo ? true : false} signOut={toggleModal} 
        // itemCount={userInfo ? userInfo.itemCount : 0}/>
        itemCount={cartCount}/>

      <Outlet context={context}/>
    
      <ModalSignOut show={modalTog} onHide={toggleModal} signOut={confirmSignOut} />
    </div>
  );
}
